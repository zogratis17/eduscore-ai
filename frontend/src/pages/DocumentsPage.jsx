...
const DocumentsPage = () => {
  ...
  const getStatusColor = (status) => {
    switch (status?.toLowerCase()) {
      ...
      case "processing":
        return "bg-indigo-50 text-indigo-700 border-indigo-200/60 ring-1 ring-indigo-500/10";
      case "failed":
      case "failed_evaluation":
        return "bg-rose-50 text-rose-700 border-rose-200/60 ring-1 ring-rose-500/10";
      default:
        return "bg-gray-50 text-gray-600 border-gray-200/60 ring-1 ring-gray-500/10";
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-sm text-gray-500 font-medium">
            Manage and view all student submissions.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <RefreshCw
              className={`h-5 w-5 ${loading ? "animate-spin text-primary-500" : ""}`}
            />
          </button>
          <Link
            to="/upload"
      ...
      {/* Documents Table */}
      <div
        className="glass bg-white rounded-2xl overflow-hidden animate-slide-up"
              ...
              {loading ? (
                ...
                filteredDocs.map((doc) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(doc.status)}`}
                      >
                        {doc.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {doc.final_score ? `${doc.final_score}/100` : "N/A"}
                    </td>
